...
const bookSection = document.getElementById("bookSection") as HTMLElement;
...
const clearCartBtn = document.getElementById("clearCartBtn") as HTMLElement;
const borrowBooksBtn = document.getElementById("borrowBooksBtn") as HTMLElement;
...
const addBookBtn = document.getElementById("addBookBtn") as HTMLElement;
...
function updateCartItemQuantity(bookId: number, change: number) {
...
// Create a new book
async function createBook(bookData: Omit<Book, "book_id">) {
  if (!currentUser) {
...
async function editBook(bookId: number) {
  ...
  if (bookFormModal && bookForm) {
    ...
    // Set form fields
    const titleInput = bookForm.querySelector("#bookTitle") as HTMLInputElement;
    const authorInput = bookForm.querySelector(
      "#bookAuthor"
    ) as HTMLInputElement;
    const genreInput = bookForm.querySelector("#bookGenre") as HTMLInputElement;
    const yearInput = bookForm.querySelector("#bookYear") as HTMLInputElement;
    const pagesInput = bookForm.querySelector("#bookPages") as HTMLInputElement;
    const publisherInput = bookForm.querySelector(
      "#bookPublisher"
    ) as HTMLInputElement;
    const descriptionInput = bookForm.querySelector(
      "#bookDescription"
    ) as HTMLTextAreaElement;
    const imageInput = bookForm.querySelector("#bookImage") as HTMLInputElement;
    const priceInput = bookForm.querySelector("#bookPrice") as HTMLInputElement;
  ...
  if (registerForm) {
    registerForm.addEventListener("submit", async (e) => {
      ...
      const role_id = parseInt(
        (document.getElementById("registerRole") as HTMLSelectElement).value
      );
  ...
  if (addBookBtn) {
    addBookBtn.addEventListener("click", () => {
      ...
      if (bookFormModal && bookForm) {
        ...
        // Set form title
        const formTitle = bookFormModal.querySelector("h2");
        if (formTitle) formTitle.textContent = "Add New Book";
...
function renderUserManagementUI(users: User[]) {
    ...
    <div class="modal-content">
      ...
      <form id="editUserForm">
        ...
        <div class="form-group">
          ...
          <input type="email" id="userEmail" name="email" value="${
            user.email
          }" required>
        </div>
        <div class="form-group">
...
function showLoading(element: HTMLElement, message: string = "Loading...") {
